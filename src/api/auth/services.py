...
async def authenticate_user(form_data: Annotated[OAuth2PasswordRequestForm, Depends()], email: str, db: AsyncSession):
    user = await UserServices.get_user(form_data.username, email, form_data.password, db)
